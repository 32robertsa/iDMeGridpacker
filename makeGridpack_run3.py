...
def run_gridpack_generation(tag):
    ...
    #The fact that I have used slc7 in place of el8 might be a good point to note for future
    cmd = './gridpack_generation.sh {0} cards/{0} local ALL el9_amd64_gcc11 CMSSW_13_0_13'.format(tag)
    print(cmd)
    os.system(cmd)
...
def lsf_submit(tag):
    ...
    os.system(cmd)
    cmd = 'mv %s_el9_amd64_gcc11_CMSSW_13_0_13_tarball.tar.xz GridPacker/GridPacks/%s.tar.xz' % (tag, tag)
    os.chdir(cwd)
...
if __name__ == "__main__":
    ...
    template = 'iDMe_run3_Mchi-XMASS_dMchi-XHS_mZDinput-MED_ctau-DLENGTH_1or2jets_icckw1_drjj0_xptj80_xqcut20'
    if oneJet:
        template = 'iDMe_run3_Mchi-XMASS_dMchi-XHS_mZDinput-MED_ctau-DLENGTH_1jet_icckw1_drjj0_xptj80_xqcut20'
    ...
    print('\tMass average: mchi = %.3f \n\tMass difference: dmchi = %.3f \n\tDark photon mass: med = %.3f' % (mchi, dmchi, med))
    ...
    tempDir = 'iDMe_run3_generic'
    if oneJet:
        tempDir = 'iDMe_run3_generic_1jet'
